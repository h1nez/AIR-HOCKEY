...
// ==========================================
// 🔥 ВИЗУАЛЬНЫЕ ЭФФЕКТЫ И УРОВНИ FACEIT
// ==========================================
let puckTrail = []; 
let confetti = []; 
let activeEmojis = []; 
...
function updateProfile() {
    socket.emit('getProfile', (data) => {
        if (data.success) {
            ...
            // Показываем кнопку админа, если есть права
            if (data.isAdmin) {
...
window.adminAction = function(targetName, action) {
    ...
    socket.emit('adminAction', { targetName, action, amount }, (res) => {
        alert(res.msg);
        if (res.success) loadAdminUsers(); // Обновляем список сразу
    });
};
...
function loadFriendsData() {
    socket.emit('getFriendsData', (res) => {
        ...
        if (res.friends.length === 0) list.innerHTML = "<p style='color:#888;'>У вас пока нет друзей :(</p>";
            ...
            list.innerHTML = res.friends.map(f => `<div class="friend-item"><div class="friend-info">${f.name} <br><span class="friend-mmr">ЭЛО: ${getLvlHtml(f.rating)} ${f.rating}</span></div><div style="display:flex; gap:5px;"><button class="btn btn-green btn-small" onclick="inviteFriendToMatch('${f.name}')">⚔️ Играть</button><button class="btn btn-blue btn-small" onclick="showProfile('${f.name}')">Профиль</button><button class="btn btn-red btn-small" onclick="removeFriend('${f.name}')">Удалить</button></div></div>`).join('');
        }
