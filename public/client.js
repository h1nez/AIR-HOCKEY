    ...
    socket.emit('getProfile', (data) => {
        if (data.success) {
            ...
            if (data.reqCount > 0) {
                ...
                reqBadge.innerText = data.reqCount;
            } else {
            ...
            if (data.title) {
                document.getElementById('title-selector').value = data.title;
            }
...
window.adminAction = function(targetName, action) {
    let amount = 0;
    if (action === 'addCoins') {
        amount = prompt(`Сколько монет дать ${targetName}? (введи число, можно с минусом)`);
        if (amount === null || isNaN(amount) || amount === "") return;
    } else if (action === 'setElo') {
        amount = prompt(`Установить новое ЭЛО для ${targetName}:`);
        if (amount === null || isNaN(amount) || amount === "") return;
    } else if (action === 'ban') {
        if (!confirm(`💀 ВНИМАНИЕ! Вы точно хотите удалить аккаунт ${targetName} НАВСЕГДА? Игрока выкинет из игры прямо сейчас.`)) return;
    }
...
window.showProfile = function(username) {
    socket.emit('getUserProfile', username, (res) => {
        if (res.success) {
            ...
            if (res.isOnline) {
                statusDot.style.backgroundColor = '#06d6a0';
...
window.setMyTitle = function(title) {
    socket.emit('setTitle', title, () => updateProfile());
};
        ...
        if (res.success && res.clan) {
            ...
            clan.members.forEach(member => {
                ...
                if (member !== myName) {
                    if (isLeader) {
                        if (isTargetDeputy) {
                            controls += `<button class="btn-promo" onclick="clanAction('${member}', 'demote')">⬇ Зам</button>`;
                        } else {
                            controls += `<button class="btn-promo" onclick="clanAction('${member}', 'promote')">⬆ Зам</button>`;
                        }
...
window.createClan = function() {
    ...
    const isPrivate = document.getElementById('new-clan-private').checked;
    
    if (!name) return alert("Введите название клана!");
    
    socket.emit('createClan', { name, maxMembers, isPrivate }, (res) => {
        alert(res.msg);
...
window.inviteToClan = function() {
    const targetName = document.getElementById('clan-invite-input').value.trim();
    if (!targetName) return;
    
    socket.emit('inviteToClan', targetName, (res) => {
        alert(res.msg);
        if (res.success) {
            document.getElementById('clan-invite-input').value = '';
        }
    });
};
...
socket.on('newClanMsg', (chatMsg) => {
    if (document.getElementById('clans-modal').style.display === 'flex') {
        const chatBox = document.getElementById('clan-chat-msgs');
        const color = chatMsg.name.includes(nameInput.value) ? '#4da6ff' : '#8338ec';
...
socket.on('incomingClanInvite', (clanName) => {
    currentClanInviter = clanName;
    document.getElementById('clan-invite-name').innerText = clanName;
    document.getElementById('clan-invite-modal').style.display = 'flex';
...
document.getElementById('btn-accept-clan-invite').onclick = () => {
    acceptClanInvite(currentClanInviter);
    document.getElementById('clan-invite-modal').style.display = 'none';
};

document.getElementById('btn-decline-clan-invite').onclick = () => {
    rejectClanInvite(currentClanInviter);
    document.getElementById('clan-invite-modal').style.display = 'none';
};

window.acceptClanInvite = function(clanName) {
    socket.emit('acceptClanInvite', clanName, (res) => {
        alert(res.msg);
        if (res.success) loadMyClan();
    });
};

window.rejectClanInvite = function(clanName) {
    socket.emit('rejectClanInvite', clanName, (res) => {
        loadMyClan();
    });
};
...
function loadFriendsData() {
    socket.emit('getFriendsData', (res) => {
        ...
        if (res.friends.length === 0) {
            list.innerHTML = "<p style='color:#888;'>У вас пока нет друзей :(</p>";
        } else {
        ...
        if (res.requests.length === 0) {
            reqList.innerHTML = "<p style='color:#888;'>Нет новых запросов</p>";
        } else {
...
window.searchFriends = function() {
    ...
    socket.emit('searchUser', q, (res) => {
        ...
        if (!res.success || res.users.length === 0) {
            resBox.innerHTML = "<p style='color:#888;'>Не найдено</p>";
            return;
        }
        resBox.innerHTML = res.users.map(u => `
            <div class="friend-item">
                <div class="friend-info">${u.name} <span class="friend-mmr">(ЭЛО: ${getLvlHtml(u.rating)} ${u.rating})</span></div>
                <div style="display:flex; gap:5px;">
...
document.getElementById('btn-accept-invite').onclick = () => {
    socket.emit('acceptInvite', currentInviter);
    document.getElementById('invite-modal').style.display = 'none';
};

document.getElementById('btn-decline-invite').onclick = () => {
    socket.emit('declineInvite', currentInviter);
    document.getElementById('invite-modal').style.display = 'none';
};

socket.on('inviteDeclined', (name) => {
    alert(`Игрок ${name} отклонил приглашение.`);
});
...
function renderShopItem() {
    ...
    if (item.id === 'default') {
        ...
        img.src = `/${item.id}.png`;
        border.style.background = '#f4faff';
    }
...
socket.on('gameStateUpdate', s => {
    ...
    if (s.paused) {
        clientState.player1.x = s.player1.x;
        clientState.player1.y = s.player1.y;
        clientState.player2.x = s.player2.x;
        clientState.player2.y = s.player2.y;
    }
});
...
function render(s) {
    ...
    ctx.textBaseline = 'middle';
    [
        {t:'🐾', x:120, y:80}, {t:'⭐', x:150, y:320}, 
        {t:'🐾', x:280, y:200}, {t:'⭐', x:350, y:80}, 
        {t:'🐾', x:400, y:320}, {t:'⭐', x:520, y:200}, 
        {t:'🐾', x:680, y:80}, {t:'⭐', x:650, y:320}
    ].forEach(d => ctx.fillText(d.t, d.x, d.y));
    
    ctx.lineWidth = 6;
    ctx.fillStyle = 'rgba(77, 166, 255, 0.2)';
    ctx.beginPath();
    ctx.arc(0, 200, 100, -Math.PI/2, Math.PI/2);
    ctx.fill();
    
    ctx.fillStyle = 'rgba(255, 77, 77, 0.2)';
    ctx.beginPath();
    ctx.arc(800, 200, 100, Math.PI/2, -Math.PI/2);
    ctx.fill();
    ...
    ctx.beginPath();
    ctx.arc(400,200,77,0,Math.PI*2);
    ctx.fill();
    ...
    ctx.lineWidth = 12;
    ctx.strokeStyle = '#4da6ff';
    ctx.strokeRect(0, 115, 10, 170);
    ctx.strokeStyle = '#ff4d4d';
    ctx.strokeRect(790, 115, 10, 170);
    ...
    if (myRole && myRole !== 'spectator' && !serverState.paused && !serverState.gameOver) {
        ...
        if (dist < pR + 22) {
            px = myPlayer.x + (dx/dist)*(pR+22);
            py = myPlayer.y + (dy/dist)*(pR+22);
        }
    }
