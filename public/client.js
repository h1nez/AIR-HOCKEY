...
function updateProfile() {
    socket.emit('getProfile', (data) => {
        if (data.success) {
            ...
            // Если есть титул, ставим его в селект
    ...
    socket.emit('adminGetUsers', (res) => {
        ...
        list.innerHTML = res.users.map(u => `
            <tr style="border-bottom: 1px solid #eee;">
                <td style="padding: 5px;"><b>${u.name}</b><br><span style="font-size:10px;color:#888;">IP: ${u.regIp || 'Скрыт'}</span></td>
...
window.showProfile = function(username) {
    socket.emit('getUserProfile', username, (res) => {
        if (res.success) {
            ...
            if (p.title) { titleLabel.innerText = `[${p.title}]`; titleLabel.style.display = 'block'; } 
            else { titleLabel.style.display = 'none'; }

            const clanLabel = document.getElementById('profile-clan-name');
            if (p.clan) { clanLabel.innerText = `🛡️ Клан: ${p.clan}`; clanLabel.style.display = 'block'; } 
            else { clanLabel.style.display = 'none'; }
            ...
            if (username === nameInput.value) {
                ...
                giftBtn.onclick = () => {
                    ...
                    if (amount && !isNaN(amount) && amount > 0) {
                        socket.emit('sendGift', { targetName: p.name, amount: amount }, (giftRes) => {
                            alert(giftRes.msg);
                            if (giftRes.success) updateProfile();
                        });
                    }
                };
            }
...
document.getElementById('btn-logout').onclick = () => {
    if (confirm("Вы уверены, что хотите выйти из аккаунта?")) {
        localStorage.removeItem('ah_name'); localStorage.removeItem('ah_pass'); window.location.reload();
    }
};
...
function loadMyClan() {
    socket.emit('getClanData', (res) => {
        if (res.success && res.clan) {
            ...
            if (isLeader || isDeputy) document.getElementById('clan-invite-block').style.display = 'block';
            else document.getElementById('clan-invite-block').style.display = 'none';
        ...
        if (res.success) {
            ...
            if (res.clans.length === 0) list.innerHTML = "<p style='color:#888; font-size:14px;'>Свободных кланов пока нет.</p>";
                ...
                list.innerHTML = res.clans.map(c => {
                    let btnHtml = c.isPrivate 
                        ? `<span style="color:#e63946; font-size: 12px; font-weight:bold;">🔒 Закрытый</span>`
...
window.spectateRoom = function(roomId) {
    ...
    document.getElementById('goal-msg').textContent = "Подключение к трансляции...";
    document.getElementById('goal-msg').style.color = "#8338ec";
    document.getElementById('btn-cancel-search').style.display = 'none';
...
document.getElementById('btn-in-game-quit').onclick = () => {
    ...
    if (myRole === 'spectator') {
        // Зритель может просто выйти
        socket.emit('leaveMatch');
        clientState = null; serverState = null; myRole = null; activeEmojis = [];
        document.getElementById('game-wrapper').style.display = 'none'; document.getElementById('end-screen').style.display = 'none';
        document.getElementById('btn-in-game-quit').style.display = 'none'; document.getElementById('main-menu').style.display = 'flex';
        updateProfile();
        return;
    }
...
socket.on('showEmoji', (data) => {
    playPop();
    let startX = 400; let startY = 200;
    if (data.role === 'p1') { startX = 150; } 
    else if (data.role === 'p2') { startX = 650; }
    else { startX = 400; startY = 50; } // Эмодзи зрителя падают сверху по центру!

    activeEmojis.push({
        text: data.emoji, x: startX + (Math.random() * 40 - 20), y: startY + (Math.random() * 40 - 20), life: 1.0
    });
});
...
function sendInput(clientX, clientY) {
    // Зритель не может двигать клюшки
...
function drawPlayer(x, y, skinName, color) {
    ...
    if (skinName && skinName !== 'default' && catImages[skinName] && catImages[skinName].complete && catImages[skinName].naturalWidth > 0) {
        ctx.save(); ctx.beginPath(); ctx.arc(x, y, r, 0, Math.PI * 2); ctx.clip(); ctx.drawImage(catImages[skinName], x - r, y - r, r * 2, r * 2); ctx.restore(); ctx.beginPath(); ctx.arc(x, y, r, 0, Math.PI * 2); ctx.lineWidth = 5; ctx.strokeStyle = color; ctx.stroke();
    } else {
        ...
        if (skinName === 'korzhik') ctx.fillText("КОРЖ", x, y); else if (skinName === 'karamelka') ctx.fillText("КАРА", x, y); else if (skinName === 'kompot') ctx.fillText("КОМП", x, y); else if (skinName === 'gonya') ctx.fillText("ГОНЯ", x, y); else { ctx.beginPath(); ctx.arc(x, y, r * 0.4, 0, Math.PI * 2); ctx.fillStyle = '#fff'; ctx.fill(); }
    }
}
...
function render(s) {
    ...
    // Зритель не просчитывает свои столкновения локально
...
function loop() {
    if (serverState && clientState) {
        ...
        // Плавное движение обоих игроков (особенно важно для зрителя)
