...
// 🔥 СИСТЕМА ТУРНИРОВ
let tourney = {
    state: 'idle', // 'idle' (ждем), 'reg' (регистрация), 'playing' (матчи идут)
    players: [], // Ники зарегистрированных
    winners: [], // Победители текущего раунда
    matchesActive: 0 // Сколько матчей сейчас играется
};
...
function createRoom(isBotMatch = false, isFriendly = false, isTournament = false) {
    ...
    rooms[roomId] = {
        ...
        botTimer: null, isBotMatch: isBotMatch, isFriendly: isFriendly, isTournament: isTournament // 🔥 Флаг турнира
    };
    return roomId;
}
...
// 🔥 ГЛАВНЫЙ ИГРОВОЙ ЦИКЛ (20 РАЗ В СЕКУНДУ)
setInterval(() => {
...
// ==========================================
// 3. АВТОРИЗАЦИЯ И ПОДБОР ИГРОКОВ
// ==========================================
...
function joinPlayerToRoom(socket, user) {
    ...
    for (const id in rooms) { 
        if (rooms[id].gameOver || rooms[id].isBotMatch || rooms[id].isFriendly || rooms[id].isTournament) continue; 
    ...
    if (!myRoomId) myRoomId = createRoom(false, false, false);
        ...
        room.botTimer = setTimeout(() => {
            if (room.player1.id && room.player2.name === "...") {
                const generateSteamName = () => {
                    const pros = ['s1mple', 'donk', 'Yatoro', 'm0NESY', 'Collapse', 'sh1ro'];
                    const prefixes = ['NaVi | ', 'Virtus.pro ', 'Team Spirit ', 'zxc ', '1000-7 '];
                    const roots = ['Ghoul', 'Sniper', 'Demon', 'Pudge', 'Pivo', 'Hokage'];
                    const suffixes = ['_pro', '2010', '228', '1337', '_rus'];
                    ...
                    else return roots[Math.floor(Math.random() * roots.length)] + suffixes[Math.floor(Math.random() * suffixes.length)];
                };
                const fakeSkins = ['default', 'korzhik', 'karamelka', 'kompot', 'gonya', 'sazhik'];
                const fakeTitles = ['', 'Новичок', 'Подпивас', 'Табуретка', 'ZXC Гуль', 'Гроза льда', 'Легенда'];
                const fakeAvatars = ['avatar1', 'avatar2', 'avatar3', 'avatar4'];
                const fakeVsEffects = ['none', 'none', 'fire', 'ice', 'neon', 'gold', 'matrix'];
...
io.on('connection', (socket) => {
    ...
    socket.on('buyBpItem', async (item, callback) => {
        ...
        try {
            ...
            if (item === 'sazhik') {
                if (u.inventory.includes('sazhik')) return callback({ success: false, msg: "Сажик уже куплен!" });
                if (u.bpXP < 1500) return callback({ success: false, msg: "Не хватает XP (нужно 1500)!" });
                u.bpXP -= 1500; u.inventory.push('sazhik'); await u.save(); socket.user = u;
                return callback({ success: true, msg: "Вы открыли эксклюзивный скин: Сажик 🐈‍⬛!" });
            }
            
            if (item === 'matrix') {
                if (u.vsEffects.includes('matrix')) return callback({ success: false, msg: "Эффект уже куплен!" });
                if (u.bpXP < 800) return callback({ success: false, msg: "Не хватает XP (нужно 800)!" });
                u.bpXP -= 800; u.vsEffects.push('matrix'); await u.save(); socket.user = u;
                return callback({ success: true, msg: "Вы открыли эффект VS-рамки: Матрица 🟩!" });
            }
...
// ==========================================
    ...
    socket.on('leaveClan', async (callback) => { 
        ...
        try { 
            ...
            if (clan) { 
                ...
                else if (clan.leader === u.name) { if (clan.deputies.length > 0) clan.leader = clan.deputies[0]; else clan.leader = clan.members[0]; await clan.save(); } 
    ...
    socket.on('clanAction', async (data, callback) => { 
        ...
        try { 
            ...
            if (data.action === 'kick') { 
                ...
                const targetUser = await User.findOne({ name: data.targetName }); if (targetUser) { targetUser.clan = null; await targetUser.save(); } 
                callback({ success: true }); 
    ...
    socket.on('sendEmoji', (emoji) => { 
        if (!socket.roomId || !rooms[socket.roomId] || !socket.user) return; 
        const room = rooms[socket.roomId]; let role = 'spectator'; 
        if (socket.id === room.player1.id) role = 'p1'; 
        else if (socket.id === room.player2.id) role = 'p2'; 
    ...
    socket.on('adminAction', async (data, callback) => { 
        ...
        try { 
            const target = await User.findOne({ name: data.targetName }); if (!target) return callback({ success: false, msg: "Игрок не найден!" }); 
            if (data.action === 'addBpXp') { await applyBP(data.targetName, Number(data.amount), connectedUsers[data.targetName]); return callback({ success: true, msg: `Успешно выдано ${data.amount} XP!` }); } 
            ...
            else if (data.action === 'ban') { await User.deleteOne({ name: data.targetName }); const tSocketId = connectedUsers[data.targetName]; if (tSocketId) { io.to(tSocketId).emit('forceReload'); const ts = io.sockets.sockets.get(tSocketId); if (ts) ts.disconnect(); } } 
    ...
    socket.on('register', async (data, callback) => { 
        try { 
            ...
            let clientIp = rawIp.split(',')[0].trim(); if (clientIp === '::1' || clientIp === '::ffff:127.0.0.1') clientIp = '127.0.0.1 (Локальный)'; 
            const hashedPassword = await bcrypt.hash(data.password, 10); 
            const newUser = new User({ name: data.name, password: hashedPassword, regIp: clientIp, vsCases: 1 }); 
            await newUser.save(); 
    ...
    socket.on('playBot', () => { 
        ...
        const roomId = createRoom(true, false, false); const room = rooms[roomId]; 
        ...
        room.paused = true; io.to(roomId).emit('showVsScreen', { p1: room.player1, p2: room.player2 }); 
        setTimeout(() => { if (rooms[roomId] && !rooms[roomId].gameOver) rooms[roomId].paused = false; }, 3000); 
    });
    ...
    socket.on('rematch', () => { 
        ...
        const room = rooms[socket.roomId]; 
        if (room.isTournament) return; // В турнире нет рематча
        ...
        if (room.rematch.player1 && room.rematch.player2) { 
            ...
            room.paused = true; io.to(room.id).emit('showVsScreen', { p1: room.player1, p2: room.player2 }); 
